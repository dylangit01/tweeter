...
$(document).ready(function () {
	// Loop tweets array and create HTML template for each tweet and append each tweet to main container
	const renderTweetsToUI = function (tweets) {
	...
	// ***Important: if condition is to make sure that only the last tweet has been prepended
	// to the main container when submitting, else rendering all
	const loadTweets = (isNewTweet) => {
		$.ajax({ url: '/tweets' }).then((data) => {
			if (isNewTweet) {
				renderLastTweet(data[data.length - 1]);
			} else {
				renderTweetsToUI(data);
			}
		});
	};

	// Load all tweets when open page at the very first time.
	loadTweets(false);
	...
	const postTweet = (event) => {
      ...
      // When POST new one tweet, only load this one, not load all again
			loadTweets(true);
		});
	};

	// Fn with "this" has to use tradition function syntax:
  ...
  // --------------- Stretch part------------------
	...
	const toggleComposeNewTweet = () => {
		...
		$sideTitle.click(() => {
			...
			// $newTweet open and close based on $sideTitle click
			$newTweet.toggle();
			$('#tweet-text').focus();
		});
